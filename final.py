...
if page == "Home":
    ...
    # GDP per capita by continent (donut)
    ...
    # 1. Top 10 Countries by GDP (Current USD)
    ...
    # 2. Mean Literacy & Unemployment Rate by Continent
    ...
    fig5 = px.bar(agg, x='Continent', y=['Literacy Rate', 'Unemployment Rate (%)'], barmode='group')
    st.plotly_chart(fig5, use_container_width=True)

    # 3. GDP of Countries with 100% Literacy Rate
    st.subheader("GDP of Countries with 100% Literacy Rate")
    ...
    # 4. Top 10 Countries by Physician Density
    st.subheader("Top 10 Countries by Physician Density")
    top_phys = df.sort_values(by="Physician Density", ascending=False).head(10)
    fig7 = px.bar(top_phys, x="Country", y="Physician Density", color="Country")
    st.plotly_chart(fig7, use_container_width=True)

    # 5. Scatter Plot: Physician Density vs Unemployment
    st.subheader("Physician Density vs Unemployment Rate")
    fig8 = px.scatter(df, x="Physician Density", y="Unemployment Rate (%)", color="Continent",
